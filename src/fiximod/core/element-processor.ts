 ...
 */
import { FixiElement } from './types.js';
import { shouldIgnore, hasFxAction } from './dom-utils.js';
import { dispatchFixiEvent } from './events.js';
import { buildConfig, getTriggerEvent } from './config-builder.js';
import { executeRequest } from './request-manager.js';
import { executeSwap } from './swap-strategies.js';
    ...
    const handler = async (event: Event) => {
      ...
      try {
        ...
        if (config.text !== undefined) {
          await executeSwap(config, config.swapMechanism);
          ...
          if (!document.contains(element)) {
            dispatchFixiEvent(document.documentElement, 'swapped', { cfg: config });
          }
        }
      } catch (error) {
